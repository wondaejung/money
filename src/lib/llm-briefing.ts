...
import { requestGeminiJsonText } from "@/lib/llm-gemini";
import {
  compressUsIndex,
  compressUsStock,
} from "@/lib/llm-payload-compress";
...
const SYSTEM_PROMPT = buildJsonSystemPrompt({
  role: "한국 개인투자자용 모닝 브리핑. 미국 증시 마감 데이터를 한국 장 개장 전 테마로 연결.",
  schema: `{
  "overnightIssues": [
...
function buildUserPrompt(context: LlmMarketContext): string {
  const indices = context.usIndices
    .map((index) =>
      compressUsIndex(index.name, index.value, index.changePercent),
    )
    .join(" ");

  const stocks = context.usStocks
    .map((stock) =>
      compressUsStock(stock.symbol, stock.name, stock.changePercent),
    )
    .join(" ");

  return `BRIEFING kst=${context.generatedAtKst}\nIDX ${indices}\nUS ${stocks}`;
}
...
async function callGemini(
  ...
  const result = await requestGeminiJsonText(systemPrompt, userPrompt);
  if (!result.text) {
